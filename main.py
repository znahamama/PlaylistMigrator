...
def load_details(config_file='config.ini'):
    config = configparser.ConfigParser()
    config.read(config_file)
    try:
        html_file_path = config.get('Anghami', 'html_file_path')
        client_id = os.getenv('SPOTIFY_CLIENT_ID', config.get('Spotify', 'client_id'))
        client_secret = os.getenv('SPOTIFY_CLIENT_SECRET', config.get('Spotify', 'client_secret'))
...
def save_playlist_to_text(songs, artists, txt_save_path, txt_song_artist_separator):
    try:
        ...
        logger.error(f"Error saving playlist to text file: {e}")
        raise
